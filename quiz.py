...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS users (
        ...
        selected_subject TEXT,  -- المادة المختارة
        selected_topic TEXT
    )''')
...
def get_question_for_user(chat_id):
    ...
    # الحصول على المادة والموضوع المختارين
    cursor.execute('SELECT selected_subject, selected_topic FROM users WHERE chat_id = ?', (chat_id,))
    result = cursor.fetchone()
    selected_subject = result[0] if result else None
    selected_topic = result[1] if result else None
    
    # الحصول على الأسئلة المجابة
    ...
    available_questions = [
        ...
        (selected_topic is None or q.get('topic', 'عام') == selected_topic) and
...
# ------ CALLBACK HANDLERS ------
@bot.callback_query_handler(func=lambda call: call.data == 'subjects_list')
@handle_errors
def handle_subjects_list(call):
    try:
        print("تم تفعيل معالج المواد الرئيسي")  # للتتبع
        chat_id = call.message.chat.id
        
        # تحميل البيانات من ملف JSON
        with open('topics_info.json', 'r', encoding='utf-8') as f:
            subjects = json.load(f)['subjects'].keys()
        
        # إنشاء أزرار المواد
        markup = types.InlineKeyboardMarkup(row_width=2)
        buttons = [types.InlineKeyboardButton(subj, callback_data=f"subject_{subj}") for subj in subjects]
        markup.add(*buttons)
        
        # إرسال القائمة مع تحديث الرسالة الأصلية
        ...
        print(f"خطأ في معالج المواد: {str(e)}")
        bot.answer_callback_query(call.id, "⚠️ فشل تحميل المواد")

@bot.callback_query_handler(func=lambda call: call.data.startswith('subject_'))
@handle_errors
def handle_subject_selection(call):
    chat_id = call.message.chat.id
    subject = call.data.split('_')[1]  # استخراج اسم المادة
    bot.answer_callback_query(call.id)
    
    # تحميل معلومات المواضيع من ملف topics_info.json
    with open('topics_info.json', 'r', encoding='utf-8') as f:
        topics_info = json.load(f)
    
    # الحصول على مواضيع المادة المحددة
    subject_topics = topics_info['subjects'].get(subject, {}).get('topics', {}).keys()
    
    if not subject_topics:
        bot.send_message(chat_id, f"⚠️ لا توجد مواضيع متاحة لمادة {subject} حالياً.")
        return
    
    # إنشاء أزرار المواضيع
    markup = types.InlineKeyboardMarkup(row_width=2)
    buttons = [types.InlineKeyboardButton(topic, callback_data=f"select_{topic}") 
              for topic in subject_topics]
    markup.add(*buttons)
    
    # إضافة زر العودة إلى قائمة المواد
    markup.add(types.InlineKeyboardButton("🔙 العودة إلى المواد", callback_data="subjects_list"))
    
    bot.send_message(
        chat_id,
        f"📖 اختر موضوعاً من مادة {subject}:",
        reply_markup=markup
    )
...
def handle_topic_selection(call):
    ...
    conn = sqlite3.connect('science_bot.db')
    cursor = conn.cursor()
    cursor.execute('''UPDATE users SET selected_topic = ?, selected_subject = ? WHERE chat_id = ?''', 
                  (topic_name, subject_name, chat_id))
    conn.commit()
    conn.close()
    
    # إرسال تفاصيل الموضوع
    response = f"✅ تم اختيار موضوع: *{topic_name}*\n\n"
    response += f"📖 الصفحات: {topic_info['pages']}\n"
    response += f"📝 الوصف: {topic_info['description']}\n"
    response += f"📊 مستوى الصعوبة: {topic_info['difficulty']}\n\n"
...
def send_welcome(message):
    ...
    # رسالة الترحيب
    response = """👋 مرحبًا بك في بوت المعلم الذكي للصف التاسع!
...
🎯 هدفي هو مساعدتك في فهم الدروس وتعزيز مهاراتك من خلال أسئلة تفاعلية."""
    ...
    markup.add(
        ...
        types.InlineKeyboardButton('📚 المواد', callback_data='subjects_list'),
...
def list_topics(message):
    ...
    if selected_subject not in topics_info['subjects']:
...
def select_topic_command(message):
    ...
    buttons = [types.KeyboardButton(topic) for topic in subject_topics]
    markup.add(*buttons)
...
def handle_unknown_message(message):
    ...
    # رسالة المساعدة
    help_text = """🆘 لم أفهم طلبك. إليك الخيارات المتاحة:

🔹 /start - عرض رسالة الترحيب
🔹 /question - الحصول على سؤال جديد
🔹 /score - عرض إحصائياتك
🔹 /topics - عرض قائمة المواضيع
🔹 /select_topic - اختيار موضوع معين
🔹 /feedback - إرسال ملاحظاتك

أو يمكنك استخدام الأزرار في لوحة التحكم أدناه:"""
    
    # إنشاء لوحة أزرار مشابهة للترحيبية
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton('🧪 سؤال جديد', callback_data='new_question'),
        types.InlineKeyboardButton('📊 إحصائياتي', callback_data='my_stats'),
