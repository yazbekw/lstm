...
def handle_subject_selection(call):
    ...
    # تحديث المادة المختارة في قاعدة البيانات
    conn = sqlite3.connect('science_bot.db')
    cursor = conn.cursor()
    cursor.execute('UPDATE users SET selected_subject = ? WHERE chat_id = ?', 
                  (subject, chat_id))
    conn.commit()
    conn.close()
    
    bot.answer_callback_query(call.id)
    bot.send_message(chat_id, f"✅ تم اختيار مادة: *{subject}*\n\n"
                             "الآن يمكنك اختيار موضوع باستخدام الأمر /topics")
...
def handle_topic_button(call):
    ...
    # البحث عن الموضوع في جميع المواد
    topic_info = None
    for subject in topics_info['subjects'].values():
        if selected_topic in subject['topics']:
            topic_info = subject['topics'][selected_topic]
            break
...
def handle_subjects_list(call):
    ...
    # عرض قائمة المواد مع أزرار للاختيار
    markup = types.InlineKeyboardMarkup(row_width=2)
    buttons = [
